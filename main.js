    ...
    fetch('documents.json')
        ...
        .then(data => {
            ...
            documentDropdown.onchange = function() {
                ...
                document.body.style.backgroundImage = `url(${image_links[doc.imgurLink]})`;
                ...
                documentDisplay.innerHTML = `
                    <div class="document-content">
                        <h2>
                            ${doc.objectID ? doc.objectID :
                            (doc.subject ? doc.subject :
                            (doc.project ? doc.project :
                            (doc.subjectID ? doc.subjectID : 'N/A')))}
                        </h2>
                        ...
                        <p><strong>Containment Procedures:</strong>
                            ${doc.containmentProcedures ? doc.containmentProcedures :
                            (doc.reportSummary ? doc.reportSummary :
                            (doc.profile ? doc.profile :
                            (doc.background ? doc.background : 'N/A')))}
                        </p>
                        <p><strong>Description:</strong>
                            ${doc.description ? doc.description :
                            (doc.details ? doc.details :
                            (doc.currentActivities ? doc.currentActivities :
                            (doc.evidence ? doc.evidence :
                            (doc.analysis ? doc.analysis :
                            (doc.operationDetails ? doc.operationDetails :
                            (doc.rehabilitationProtocol ? doc.rehabilitationProtocol : 'N/A'))))))}
                        </p>
                        ${doc.procedures ? Object.entries(doc.procedures).map(([key, value]) =>
                        ...
                        <ul>
                            ${doc.addendums && doc.addendums.length > 0 ? doc.addendums.map(addendum => `<li>${addendum}</li>`).join('') : '<li>N/A</li>'}
                        </ul>
                    </div>
                `;
            };
